...
def RDFprocess(fvec, outputDir, sample, xsec, systType, sumwClipped, pretendJob):
    ...
    if systType == 0: #this is data
        ...
        p.branch(nodeToStart = 'defs', nodeToEnd = 'defs', modules = [ROOT.customizeforUL(True,False), ROOT.recoDefinitions(True, False),ROOT.recoWeightDefinitions(fileSF), getLumiWeight(xsec=xsec, inputFile = fvec, genEvsbranch = "genEventSumw", targetLumi = 19.3)]) #

        p.Histogram(columns = ["Mu1_eta","Mu1_pt","Mu1_charge","MTVars","Mu1_relIso", "lumiweight", "PrefireWeight", "puWeight", "WHSFVars"], types = ['float']*9,node='defs',histoname=ROOT.string('ewk'),bins = [etaBins,ptBins,chargeBins,mTBins,isoBins], variations = [])
    else:
        if 'DY' in sample: #reweight full Z kinematics
            p.branch(nodeToStart = 'defs', nodeToEnd = 'defs', modules = [ROOT.customizeforUL(True, True), ROOT.recoDefinitions(True, False),ROOT.recoWeightDefinitions(fileSF),ROOT.lumiWeight(xsec=xsec, sumwclipped=sumwClipped, targetLumi = 19.3),ROOT.Replica2Hessian(),ROOT.reweightFromZ(filePt,fileY,False,True)])
            ...
            p.branch(nodeToStart = 'defs', nodeToEnd = 'defs', modules = [ROOT.customizeforUL(True, True), ROOT.recoDefinitions(True, False),ROOT.recoWeightDefinitions(fileSF),ROOT.lumiWeight(xsec=xsec, sumwclipped=sumwClipped, targetLumi = 19.3),ROOT.Replica2Hessian(),ROOT.reweightFromZ(filePt,fileY,True,True)])
